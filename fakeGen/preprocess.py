...
def generate_data(review_path, label_path):
    ...
    train_data.to_csv(os.path.join(path, 'train_data.csv'), index=None)
    test_data.to_csv(os.path.join(path, 'test_data.csv'), index=None)

    # fake data
    fake_data = train_data[train_data['label'] == -1]
    fake_data.loc[:, 'tgt'] = fake_data['src']
    fake_data.to_csv(os.path.join(path, 'fake_data.csv'), index=None)

    # real data
    real_data = train_data[train_data['label'] == 1]
    real_data.to_csv(os.path.join(path, 'real_data.csv'), index=None)
